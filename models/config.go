...
func initConfig() {
	...
	for _, name := range []string{"app.conf", "config.yaml", "reply.php"} {
		...
		if len(s) == 0 {
			...
			r, err := httplib.Get(GhProxy + "https://raw.githubusercontent.com/sxsto/dmm/main/conf/demo_" + name).Response()
			if err == nil {
				io.Copy(f, r.Body)
			}
		}
	...
	if ExecPath == "/Users/sxsto/Desktop/dmm" || Config.NoAdmin {
		Sxsto = true
	}
	if Config.Master == "" {
		Config.Master = "xxxx"
	...
	if Config.Database == "" {
		Config.Database = ExecPath + "/.dmm.db"
	}
	if Config.Npm == "" {
		Config.Npm = "npm"
	}
