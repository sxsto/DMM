...
func (c *Container) write(cks []JdCookie) error {
	...
	case "ql":
		if c.Version == "2.8" || c.Version == "2.9" {
			if len(c.Delete) > 0 {
				c.request("/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
			}
			hh := []string{}
			if len(cks) != 0 {
				for _, ck := range cks {
				...
				c.request("/envs", POST, fmt.Sprintf(`[%s]`, strings.Join(hh, ",")))
				...
				for k := range help {
					...
					if len(toDelete) > 0 {
						c.request("/envs", DELETE, fmt.Sprintf(`[%s]`, strings.Join(toDelete, ",")))
					}
				}
				for k, v := range help {
					if v == "" {
					...
					d, _ := json.Marshal(r)
					c.request("/envs", POST, fmt.Sprintf(`[%s]`, string(d)))
				}

			}
		} else {
			if len(c.Delete) > 0 {
				c.request("/cookies", DELETE, fmt.Sprintf(`[%s]`, strings.Join(c.Delete, ",")))
			}
			d := []string{}
			for _, ck := range cks {
				if ck.Available == True {
			...
			if len(d) != 0 {
				c.request("/cookies", POST, fmt.Sprintf(`[%s]`, strings.Join(d, ",")))
			}
		}
	case "v4":
